...
local function drawBorder(x, y, width, height)
    ...
    drawRect(x, width, -border_x, height, colour.border) --left
...
while true do
    if NETWORK.NETWORK_IS_SESSION_STARTED() then
        ...
        if ((focused[1] ~= nil and focused[2] == nil) or render_window) and menu.is_open() then
            ...
            local map_w_total = (map_w + padding_x * 3) * (1 + 1/22)
            ...
            --map
            directx.draw_texture(textures.map, map_w/2, 0, 0, 0, map_x + padding_x * 2 + map_w_total/22, player_list_y, 0, 1, 1, 1, 0.75)

            --blip
            directx.draw_texture(textures.blip, 0.004, 0, 0.5, 0.5, map_x + padding_x * 2 + map_w_total/22 + ((player_pos.x + 4000)/8500) * map_w, player_list_y + (1 - (player_pos.y + 4000)/12000) * gui_h, (360 - heading)/360, colour.blip)
            ...
            drawRect(map_x + padding_x, player_list_y + gui_h/2 - padding/2, map_w_total/22, -((gui_h - padding * 3)/2 * armour_perc), colour.armour_bar) --foreground
            drawRect(map_x + padding_x, player_list_y + padding, map_w_total/22, (gui_h - padding * 3)/2 * (1 - armour_perc), armour_bar_bg) --background
            ...
            drawRect(map_x + padding_x, player_list_y + gui_h - padding, map_w_total/22, -((gui_h - padding * 3)/2 * health_perc), colour.health_bar) --foreground
            drawRect(map_x + padding_x, player_list_y + gui_h/2 + padding/2, map_w_total/22, (gui_h - padding * 3)/2 * (1 - health_perc), health_bar_bg) --background
        end
    end
    util.yield()
end